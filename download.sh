...
# latest release
latest_release_stable=$(curl -s https://api.github.com/repos/${repo_owner}/venus-os_${driver_name}/releases/latest | grep "tag_name" | cut -d : -f 2,3 | tr -d "\ " | tr -d \" | tr -d \,)

# nightly build
latest_release_nightly=$(curl -s https://raw.githubusercontent.com/${repo_owner}/venus-os_${driver_name}/master/${driver_name}/${driver_name}.py | grep HardwareVersion | awk -F'"' '{print $4}')
...
if [ "$version" = "latest release \"$latest_release_stable\"" ]; then
    ...
    url=$(curl -s https://api.github.com/repos/${repo_owner}/venus-os_${driver_name}/releases/latest | grep "zipball_url" | sed -n 's/.*"zipball_url": "\([^"]*\)".*/\1/p')
fi

## nightly build
if [ "$version" = "nightly build \"v$latest_release_nightly\"" ]; then
    # download nightly build
    url="https://github.com/${repo_owner}/venus-os_${driver_name}/archive/refs/heads/master.zip"
fi
